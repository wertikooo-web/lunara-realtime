...
## Главное правило изменений (Lunara Core Contract)

- ИИ может свободно менять реализацию внутри существующего контракта.
- ИИ не может самовольно менять границы модулей, состояния, протоколы и структуру данных.
- Не допускается без отдельного явного согласования с пользователем:
  - Добавлять новые состояния realtime-сессий;
  - Менять формат или тип WebSocket-сообщений;
  - Добавлять новые поля в конфигурации устройств;
  - Менять схемы и таблицы базы данных;
  - Переносить ответственность или логику между прошивкой ESP32 и сервером;
  - Добавлять скрытые preload-модули;
  - Создавать вторые параллельные аудиопайплайны;
  - Менять поведение аппаратных/интерфейсных кнопок или внутреннюю логику завершения реплик (turn).
