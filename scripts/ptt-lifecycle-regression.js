...
async function main() {
    ...
    try {
        ...
        if (provider.sessions.length !== beforeTurnClosedSessions + 1) {
            throw new Error('Provider turn-closed-during-input must rotate exactly one provider session');
        }
        if (turnClosedRotation.old_provider_instance_id === turnClosedRotation.new_provider_instance_id) {
            throw new Error('Provider turn-closed-during-input recovery must create a new provider session');
        }
