...
Дата аудита: 2026-07-13. Обновлено: 2026-07-19 (исходящие `audio.chunk` ограничены по умолчанию 2800 байтами PCM, чтобы полный JSON гарантированно помещался в 4-КБ буфер ESP32).
...
- **Выход (сервер → ESP32):** PCM16LE, 24000 Hz, mono, внутри `audio.chunk` как `audio_base64` (Base64), **JSON-фрейм** (opcode `0x1`), НЕ binary frame. Сервер делит крупные ответы Gemini на фрагменты не более 2800 байт PCM каждый, не меняя JSON-схему.
