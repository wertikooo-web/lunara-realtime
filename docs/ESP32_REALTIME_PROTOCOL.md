...
Дата аудита: 2026-07-13. Обновлено: 2026-07-17 (найдена и исправлена реальная причина, по которой ESP32 не видел `session.ready`/`session.config.applied` — размер пакета ~23КБ превышал буфер приёма embedded WS-клиента; см. changelog в конце файла).
