...
Дата аудита: 2026-07-13. Обновлено: 2026-07-19 (исходящие `audio.chunk` ограничены по умолчанию 4096 байтами PCM, чтобы полный JSON с запасом помещался в 8-КБ буфер ESP32 или обрабатывался динамической сборкой).
...
- **Выход (сервер → ESP32):** PCM16LE, 24000 Hz, mono, внутри `audio.chunk` как `audio_base64` (Base64), **JSON-фрейм** (opcode `0x1`), НЕ binary frame. Сервер делит крупные ответы Gemini на фрагменты не более 4096 байт PCM каждый, не меняя JSON-схему.
...
| `audio.chunk` | `chunk_index`, `mime_type: 'audio/pcm'`, `sample_rate: 24000`, `audio_base64`, `elapsed_ms` | Каждый чанк аудио ответа; по умолчанию не более **4096 байт декодированного PCM** (~5464 символа Base64, полный JSON около 5.6 КБ) | Декодировать и проигрывать строго по `chunk_index` |
...
- **Максимальный исходящий чанк:** `MAX_OUTBOUND_PCM_CHUNK_BYTES = 4096` (настраивается на сервере через `REALTIME_MAX_OUTBOUND_PCM_CHUNK_BYTES`). Более крупный PCM от Gemini сервер декодирует и делит на последовательные `audio.chunk`; каждый фрагмент сохраняет PCM16LE-выравнивание, а `chunk_index` увеличивается для каждого фактически отправленного фрагмента. При 24 kHz mono это примерно 85.3 мс аудио на полный чанк. Вместе с Base64 и служебными полями полный JSON остаётся меньше типичного 8-КБ приёмного буфера ESP32.
- **Требование к ESP32:** парсить WebSocket TEXT payload по переданной библиотекой длине, не через `strlen()` и не предполагая завершающий `\0`. Если библиотека отдаёт одно WS-сообщение несколькими callback-фрагментами, сначала собрать его полностью по её признакам `final/index/total length` (рекомендуется динамическая аллокация буфера под размер `event->payload_len` во избежание статических лимитов памяти), и только затем запускать JSON parser.
