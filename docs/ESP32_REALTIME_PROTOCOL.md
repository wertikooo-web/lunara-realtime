...
Дата аудита: 2026-07-13. Обновлено: 2026-07-14 (Protocol V2 — переход ESP32 на единый режим 24/24, серверный ресемплинг входа; см. changelog в конце файла).
...
Версия протокола изменилась с V1. Причина: кодек ESP32 подключён к микрофону и динамику через один общий I2S-интерфейс на одной частоте семплирования — нельзя задать разные частоты для записи и воспроизведения на одном кодеке. Поэтому вместо асимметричного 16/24 на устройстве теперь единый режим 24/24, а ресемплинг входа 24→16кГц для Gemini выполняет **сервер**, потоково, до того как аудио попадает в `realtimeServer.js` (см. `src/realtime/inputResampleBootstrap.js`).

- **Вход (ESP32 → сервер):** PCM16LE, **24000 Hz**, mono, без обёртки, **binary WebSocket frame** (opcode `0x2`).
- **Обязательное поле в `session.start`:** `sampleRate: 24000` (или `sample_rate: 24000`) — без этого поля сервер по умолчанию считает вход уже 16kHz и НЕ ресемплирует, что на реальных 24kHz-байтах даст испорченный/ускоренный звук на входе у Gemini. Это изменение относительно более старой версии этого документа, где `sampleRate` числился как мёртвое, игнорируемое поле — теперь это не так, см. раздел 9.
- Сервер ресемплирует поток (FIR-фильтр 47 тапов + линейная интерполяция, `src/realtime/pcm16Resampler.js`) перед отправкой в Gemini Live (которому нужен вход 16000 Hz); состояние ресемплера привязано к одному turn'у: сбрасывается на `input_audio.start` (начало нового turn), полностью сливается (`flush()`) на `input_audio.end`, и сбрасывается на `session.interrupt` и при ошибке декодирования.
- **Выход (сервер → ESP32):** PCM16LE, 24000 Hz, mono, внутри `audio.chunk` как `audio_base64` (Base64), **JSON-фрейм** (opcode `0x1`), НЕ binary frame. Не изменился — сервер просто ретранслирует ответ Gemini как есть.
...
- Совместимость: старые клиенты (Browser Lab, mock), которые не шлют `sampleRate` или шлют `16000`, продолжают работать как раньше — сервер просто пропускает вход без ресемплинга (pass-through), поведение не изменилось.
...
- **Необязательные поля, которые сервер читает:**
  ...
  - `sampleRate` / `sample_rate` (number, `16000` или `24000`) — **начиная с Protocol V2 это больше не мёртвое поле.** Читается не самим `realtimeServer.js`, а внешней обёрткой `src/realtime/inputResampleBootstrap.js` (подключена флагом `-r` в `package.json`, см. врез выше), которая настраивает по этому полю потоковый ресемплинг входа. Если не передать это поле, сервер считает вход уже 16000Hz — для ESP32 в режиме 24/24 это поле обязательно передавать.
...
|---|---|---|---|
| `session.ready` | `session_id`, `provider`, `provider_instance_id`, `rotation_mode`, `model`, `config`, `lab_prompt` | Сразу на коннект (`realtimeServer.js:1399-1408`) | Держать кнопку заблокированной до `session.config.applied` (см. раздел 3) |
| `session.config.applied` | `reason`, `prompt_source`, `lab_prompt.{allow_custom_prompt,max_chars,current_context,meta}` | После обработки `session.start` (`realtimeServer.js:446-473`) | **Разблокировать PTT-кнопку здесь**, не на `session.ready` — персонализация (голос/промт/память) уже применена |
...
- **2026-07-14 (3) — Protocol V2:** ESP32 переведён на единый режим PCM16LE 24000 Hz mono на входе (кодек+I2S работают на одной частоте для микрофона и динамика — нельзя задать 16/24 асимметрично на одном кодеке). Сервер теперь потоково ресемплирует вход 24→16kHz перед Gemini (`src/realtime/pcm16Resampler.js` — FIR 47 тапов + линейная интерполяция; `src/realtime/inputResampleBootstrap.js` — обёртка вокруг `wsProtocol.createFrameParser`, подключена флагом `-r` в `package.json`). Состояние ресемплера сбрасывается на `input_audio.start`, сливается на `input_audio.end`, сбрасывается на `session.interrupt` и на ошибке декодирования (реализовано и покрыто тестом `scripts/input-resample-bootstrap-smoke.js`). Выход Gemini (24kHz) не изменился. **Критичное исправление:** поле `sampleRate`/`sample_rate` в `session.start`, ранее в этом документе описанное как мёртвое и игнорируемое сервером — теперь читается обёрткой-ресемплером и обязательно для ESP32 (`24000`); исправлено везде, где документ ранее утверждал обратное (разделы 4.1, 13). Тесты: `npm run smoke:pcm-resampler` (идентичность потокового/целого прогона = отсутствие щелчков на границах чанков, корректная длина/длительность вывода, подавление частот выше Найквиста) и `npm run smoke:input-resample-bootstrap` (интеграция с обёрткой, независимость последовательных turn'ов, восстановление после ошибки декодирования на следующем turn) — оба зелёные.
